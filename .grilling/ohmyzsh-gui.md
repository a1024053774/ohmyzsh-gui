...
- E17 | state: observed | source: code review + regression tests 2026-09-14 | Applied operations now report `undo_available: false` until the restore implementation exists; plugin removal says manual restore is required, avoiding a false Undo promise. Nine Rust tests and the native `.app` build still pass.
