...
- E17 | state: superseded | source: code review + regression tests 2026-09-14 | The earlier all-disabled Undo semantics were replaced by tested restore support for configuration, install, and remove; update remains explicitly non-undoable.
- E18 | state: observed | source: CUA native app inspection 2026-09-14 | The exact rebuilt app now has a dedicated Installed sidebar screen showing the user's official and custom plugin inventory, enabled badges, and detail actions.
- E19 | state: observed | source: isolated filesystem test + native build 2026-09-14 | Ten Rust tests pass, including a configuration Undo test that restores the backup and consumes the journal entry; the native `.app` rebuild passes. Activity exposes Undo for supported operations.
- E20 | state: implemented | source: code review + serialization coverage 2026-09-14 | Update journal entries now persist both current and previous commit SHA; update Undo verifies the checkout is still at the recorded target before fetching and restoring the previous commit. Live remote checkout rollback remains unrun.
- E21 | state: observed | source: isolated filesystem regression 2026-09-14 | Eleven Rust tests pass, including the guard that refuses install Undo while the plugin is still enabled in `.zshrc`; this prevents a dangling plugin reference.
- E22 | state: observed | source: ignored live GitHub lifecycle smoke test 2026-09-14 | `cargo test -- --ignored` passed a temporary-HOME smoke test against `syi0808/shellsuggest`: real SHA install, checkout rollback/update, update Undo to the previous SHA, remove with config disable, and remove Undo. No user HOME or live `.zshrc` was touched.
- E23 | state: observed | source: local CI definition + frontend check 2026-09-14 | The CI matrix covers macOS, Ubuntu, and Windows with Node syntax, Rust format, `cargo check`, and offline tests; the Installed/Discover selection guard passes `node --check` and is included in the next native build.
- E24 | state: observed | source: local release build 2026-09-14 | `cargo tauri build --bundles app` completed an optimized release build and produced `/Users/luckye/Documents/ohmyzsh-gui/src-tauri/target/release/bundle/macos/ohmyzsh-gui.app`.
- E25 | state: incomplete | source: local cross-target probes 2026-09-14 | Linux and Windows Rust targets installed, but cross-target checks stopped in native dependency build because this Mac lacks `x86_64-linux-gnu-gcc`, Linux pkg-config/sysroot, and `x86_64-w64-mingw32-gcc`. No source compile error appeared before those environment requirements.
- E26 | state: observed | source: CUA native release app inspection 2026-09-14 | The optimized release bundle launched successfully, loaded the real `/Users/luckye/.zshrc`, and rendered the Apple-inspired sidebar, cards, safety state, and status bar without mutation.
- E27 | state: observed | source: frontend audit + release rebuild 2026-09-14 | Uninstalled GitHub search results no longer expose Enable; the action is available only after the checkout is installed, preventing a dangling `.zshrc` reference. The optimized release bundle was rebuilt after this fix.
- E28 | state: observed | source: CUA optimized release app + filesystem readback 2026-09-14 | A real no-op Apply and Undo on `/Users/luckye/.zshrc` completed through the UI. Activity recorded both operations, and the final `.zshrc` matched the latest restore backup byte-for-byte by SHA-256 and `cmp`.
- E29 | state: observed | source: CUA native app + live GitHub search 2026-09-14 | Discover now auto-loads recommendations sorted by stars and shows source filters, sort controls, descriptions, star counts, license, and updated time. The rebuilt app displayed `awesome-zsh-plugins` at 18,002 stars with BSD-3-Clause metadata and a Review install action.
