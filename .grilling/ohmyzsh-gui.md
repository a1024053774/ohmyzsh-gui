...
- E19 | state: observed | source: isolated filesystem test + native build 2026-09-14 | Ten Rust tests pass, including a configuration Undo test that restores the backup and consumes the journal entry; the native `.app` rebuild passes. Activity now exposes Undo for supported operations.
